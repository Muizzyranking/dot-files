    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd("LspAttach", {
        ...
        callback = function(event)
          ...
          local keymap = function(key, func, desc, icon)
            local ic
            if type(icon) == "string" or "table" then
              ic = icon
          ...
          if client and client.supports_method(vim.lsp.protocol.Methods.textDocument_codeAction) then
            keymap("<leader>ca", vim.lsp.buf.code_action, "Code Action", { icon = " ", color = "orange" })
          end
          if client and client.supports_method(vim.lsp.protocol.Methods.textDocument_inlayHint) then
            keymap(
              ...
              function()
                ...
                if enabled then
                  notify.warn("Inlay Hints disabled", { timeout = 2000, title = "LSP" })
                else
                  notify.info("Inlay Hints enabled", { timeout = 2000, title = "LSP" })
                end
              end,
              "Toggle inlay_hint",
              function()
