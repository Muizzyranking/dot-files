Utils.map.add_to_wk({
	{ "<leader>l", desc = "LSP", icon = { icon = " ", color = "orange" } },
})
return {
	{
		"gd",
	...
	},
	{
		"<leader>uC",
		get = function(buf)
			return vim.lsp.document_color.is_enabled({ bufnr = buf })
		end,
		set = function(state, buf)
			vim.lsp.document_color.enable(not state, { bufnr = buf })
		end,
		name = "Document Color",
		has = "color",
	},
